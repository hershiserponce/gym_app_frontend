      ...
      <section className="relative mx-auto grid w-full max-w-7xl items-center gap-14 px-6 pb-20 pt-12 lg:grid-cols-[0.9fr_1.1fr] lg:px-10 lg:pb-28 lg:pt-20">
        ...
        <div className="relative lg:pl-8">
          ...
          <div className="relative rotate-2 rounded-[2rem] border border-white/10 bg-[#151a16] p-3 shadow-2xl shadow-black/50 transition hover:rotate-0">
            <div className="rounded-[1.5rem] bg-[#f4f3ed] p-5 text-[#101410] sm:p-7">
              <div className="mb-8 flex items-center justify-between"><div><p className="text-xs font-bold uppercase tracking-widest text-[#76806f]">Resumen del gym</p><h2 className="mt-1 text-2xl font-black">Buenos días, Alex</h2></div><div className="flex size-10 items-center justify-center rounded-full bg-[#d9f59b] text-sm font-black">AG</div></div>
              <div className="grid grid-cols-2 gap-3 sm:grid-cols-3"><div className="rounded-2xl bg-[#d9f59b] p-4"><Users className="mb-5 size-5" /><p className="text-3xl font-black">1,248</p><p className="mt-1 text-xs font-bold text-[#536039]">Miembros activos</p></div><div className="rounded-2xl bg-[#e6e8e1] p-4"><BarChart3 className="mb-5 size-5" /><p className="text-3xl font-black">$18.4k</p><p className="mt-1 text-xs font-bold text-[#76806f]">Ingresos este mes</p></div><div className="col-span-2 rounded-2xl bg-[#171c17] p-4 text-white sm:col-span-1"><LayoutDashboard className="mb-5 size-5 text-[#c8f169]" /><p className="text-3xl font-black">+24%</p><p className="mt-1 text-xs font-bold text-[#aeb4ac]">Crecimiento</p></div></div>
              <div className="mt-4 rounded-2xl border border-[#dfe2d9] p-5"><div className="mb-5 flex justify-between"><p className="font-bold">Rendimiento mensual</p><span className="text-xs font-bold text-[#75806e]">2024</span></div><div className="flex h-28 items-end gap-2 sm:gap-4">{[35, 52, 43, 70, 58, 82, 96, 76, 100, 88, 94, 100].map((height, index) => <div key={index} className={`flex-1 rounded-t-md ${index > 8 ? 'bg-[#c8f169]' : 'bg-[#d8ded0]'}`} style={{ height: `${height}%` }} />)}</div></div>
            </div>
          </div>
